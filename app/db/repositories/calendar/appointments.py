...
class AppointmentRepository:
    ...
    def get_all_appointments(self) -> List[Appointments]:
        """Получить все записи на прием из базы данных"""
        return self.db.query(Appointments).all()
    
    def get_appointment_by_id(self, appointment_id: int) -> Appointments:
        """Получить запись на прием по ID"""
        return self.db.query(Appointments).filter(Appointments.id == appointment_id).first()
    ...
    def update_appointment(self, appointment_id: int, appointment: AppointmentUpdate) -> Appointments:
        """Обновить запись на прием"""
        db_appointment = self.get_appointment_by_id(appointment_id)
        if db_appointment:
